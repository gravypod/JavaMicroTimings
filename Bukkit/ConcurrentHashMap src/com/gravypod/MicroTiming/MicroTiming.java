...
public class MicroTiming {
	...
	static ConcurrentHashMap<Long, String> listTest = new ConcurrentHashMap<Long, String>();
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < how_many; i++) {
			...
			for (String s : possible_commands) {
				...
				listTest.put(Long.parseLong(Integer.toString(s.getBytes().length)), s);
				
				for (Long s1 : listTest.keySet()) {
					
					if (listTest.get(s1).equals(s)) 
						break;
